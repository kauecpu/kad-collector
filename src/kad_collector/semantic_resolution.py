...
def decide_document_version(
    ...
    if profile.identity_key is None or profile.has_conflict:
        return IdentityResolution(
            outcome="uncertain", profile=profile,
    ...
    if same_content:
        ...
        return IdentityResolution(
            outcome="republication", profile=profile, document_version_id=winner.version_id,
            predecessor_version_id=winner.predecessor_version_id,
            version_number=winner.version_number, reason="conteúdo equivalente já registrado",
        )
    if same_identity_and_role:
        predecessor = same_identity_and_role[-1]
        return IdentityResolution(
            outcome="new_version", profile=profile,
    ...
    return IdentityResolution(
        outcome="new_identity", profile=profile, version_number=1,
        reason="primeira versão para identidade e papel",
    )


def _known_versions(connection: sqlite3.Connection, profile: DocumentSemanticProfile) -> tuple[KnownDocumentVersion, ...]:
    if profile.identity_key is None:
        return ()
    rows = connection.execute(
        "SELECT id, identity_key, document_role, content_sha256, version_number, predecessor_version_id "
        "FROM document_versions WHERE identity_key = ? AND document_role = ? ORDER BY version_number",
        (profile.identity_key, profile.document_role),
    ).fetchall()
    return tuple(KnownDocumentVersion(
        version_id=row["id"], identity_key=row["identity_key"], document_role=row["document_role"],
        content_sha256=row["content_sha256"], version_number=row["version_number"],
...
def _event(connection: sqlite3.Connection, *, document_id: str, version_id: str | None,
           action: str, payload: dict[str, Any], resolved_at: str) -> None:
    ...
    connection.execute(
        ...
        "(event_key, document_id, document_version_id, action, actor, algorithm_version, payload_json, created_at) "
        "VALUES (?, ?, ?, ?, 'system', ?, ?, ?)",
        (key, document_id, version_id, action, IDENTITY_ALGORITHM_VERSION, canonical_json(payload), resolved_at),
    )
...
def resolve_document_version(
    ...
    try:
        ...
        if decision.outcome == "uncertain":
            ...
            _event(connection, document_id=document_id, version_id=None, action="uncertain",
                   payload={"reason": decision.reason}, resolved_at=resolved_at)
            if owns_transaction:
                connection.commit()
            return decision
        ...
        connection.execute(
            ...
            "(identity_key, schema_version, algorithm_version, identity_json, evidence_json, created_at, updated_at) "
            "VALUES (?, 1, ?, ?, ?, ?, ?)",
            (profile.identity_key, IDENTITY_ALGORITHM_VERSION, identity_json, canonical_json(evidence), resolved_at, resolved_at),
        )
        ...
        if decision.outcome == "new_version":
            connection.execute(
                ...
                "(id, identity_key, document_role, answer_key_state, coverage_json, profile_json, content_sha256, "
                "content_normalizer_version, version_number, predecessor_version_id, created_at, updated_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (version_id, profile.identity_key, profile.document_role, profile.answer_key_state,
                 canonical_json(profile.coverage.model_dump(mode="json")), canonical_json(profile.model_dump(mode="json")),
                 profile.content_fingerprint.sha256, profile.content_fingerprint.normalizer_version,
                 version_number, predecessor, resolved_at, resolved_at),
            )
        elif decision.outcome == "new_identity":
            connection.execute(
                "INSERT OR IGNORE INTO document_versions "
                "(id, identity_key, document_role, answer_key_state, coverage_json, profile_json, content_sha256, "
                "content_normalizer_version, version_number, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (version_id, profile.identity_key, profile.document_role, profile.answer_key_state,
                 canonical_json(profile.coverage.model_dump(mode="json")), canonical_json(profile.model_dump(mode="json")),
                 profile.content_fingerprint.sha256, profile.content_fingerprint.normalizer_version,
                 version_number, resolved_at, resolved_at),
            )
        ...
        connection.execute(
            "UPDATE documents SET document_version_id = ?, semantic_resolution = ?, updated_at = ? WHERE id = ?",
            (version_id, decision.outcome, resolved_at, document_id),
        )
        connection.execute(
            "UPDATE document_observations SET document_id = ?, document_version_id = ?, resolution_status = ? "
        ...
        _event(connection, document_id=document_id, version_id=version_id,
               action=decision.outcome, payload={"identityKey": profile.identity_key, "versionNumber": version_number},
               resolved_at=resolved_at)
        result = decision.model_copy(update={"document_version_id": version_id,
